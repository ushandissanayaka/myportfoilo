...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class CommonClass {
      constructor(){this.width=0;this.height=0;this.pixelRatio=1;this.container=null;this.renderer=null;this.clock=null;this.time=0;this.delta=0;}
      init(container){
    ...
    class MouseClass {
      constructor(){
        ...
        this.diff=new THREE.Vector2();this.timer=null;this.container=null;this.listenerTarget=null;
        this.docTarget=null;this.isHoverInside=false;this.hasUserControl=false;this.isAutoActive=false;
    ...
    class ExternalForce extends ShaderPass {
      constructor(p){
        ...
        const mm=new THREE.RawShaderMaterial({vertexShader:mouse_vert,fragmentShader:externalForce_frag,blending:THREE.AdditiveBlending,depthWrite:false,uniforms:{px:{value:p.cellScale},force:{value:new THREE.Vector2(0,0)},center:{value:new THREE.Vector2(0,0)},scale:{value:new THREE.Vector2(p.cursor_size,p.cursor_size)}}});
        this.mouse=new THREE.Mesh(mg,mm);this.scene.add(this.mouse);
      }
      update(p){
        const fx=(Mouse.diff.x/2)*p.mouse_force,fy=(Mouse.diff.y/2)*p.mouse_force;
        ...
        const u=this.mouse.material.uniforms;u.force.value.set(fx,fy);u.center.value.set(cx,cy);u.scale.value.set(p.cursor_size,p.cursor_size);super.update();
      }
    }
    ...
    class Viscous extends ShaderPass {
      ...
      update({viscous,iterations,dt}){
        ...
        for(let i=0;i<iterations;i++){fbo_in=i%2===0?this.props.output0:this.props.output1;fbo_out=i%2===0?this.props.output1:this.props.output0;this.uniforms.velocity_new.value=fbo_in.texture;this.props.output=fbo_out;this.uniforms.dt.value=dt;super.update();}
        return fbo_out;
      }
    }
    ...
    class Poisson extends ShaderPass {
      ...
      update({iterations}){
        let p_in,p_out;
        for(let i=0;i<iterations;i++){p_in=i%2===0?this.props.output0:this.props.output1;p_out=i%2===0?this.props.output1:this.props.output0;this.uniforms.pressure.value=p_in.texture;this.props.output=p_out;super.update();}
        return p_out;
      }
    }
    ...
    class Simulation {
      ...
      update(){
        this.boundarySpace[this.options.isBounce?'set':'copy'](this.options.isBounce?[0,0]:this.cellScale);
        if(!this.options.isBounce)this.boundarySpace.copy(this.cellScale); else this.boundarySpace.set(0,0);
    ...
    const io=new IntersectionObserver(entries=>{
      ...
      isVis&&!document.hidden?webglRef.current.start():webglRef.current.pause();
